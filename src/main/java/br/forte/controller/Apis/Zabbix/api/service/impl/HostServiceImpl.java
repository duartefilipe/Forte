...
//			}
	...
	@Override
	public String createHost(Host host, HostInterface hostInterface, HostGroup hostgroup, Template template, Macro macro){
		String result = "";
		...
		if(host!=null){
			...
			if(macro!=null){
				...
				if(macro.getMacro()!=null&&!"".equals(macro.getMacro())){
					mymacro.setMacro(macro.getMacro());
				}
				if(macro.getValue()!=null&&!"".equals(macro.getValue())){
					mymacro.setValue(macro.getValue());
				}
				create.getParams().getMacros().add(mymacro);
			}else{
				return "Error,Macro is required";
			}
...
//			return "success";
	...
	@Override
	public String alteraHost(Host host, HostInterface hostInterface, HostGroup hostgroup, Template template, Macro macro){
		String result = "";
		HostUpdateRequest update = new HostUpdateRequest();
		if(host!=null){
			if(host.getHost()!=null&&!"".equals(host.getHost())){
                update.getParams().setHost(host.getHost());
			}
			if(host.getAvailable()!=null){
                update.getParams().setAvailable(host.getAvailable());
			}
			if(host.getDisable_until()!=null&&!"".equals(host.getDisable_until())){
                update.getParams().setDisable_until(host.getDisable_until());
			}
			if(host.getError()!=null){
                update.getParams().setError(host.getError());
			}
			if(host.getErrors_from()!=null&&!"".equals(host.getErrors_from())){
                update.getParams().setErrors_from(host.getErrors_from());
			}
			if(host.getIpmi_authtype()!=null){
                update.getParams().setIpmi_authtype(host.getIpmi_authtype());
			}
			if(host.getIpmi_available()!=null){
                update.getParams().setIpmi_available(host.getIpmi_available());
			}
			if(host.getIpmi_disable_until()!=null&&!"".equals(host.getIpmi_disable_until())){
                update.getParams().setIpmi_disable_until(host.getIpmi_disable_until());
			}
			if(host.getIpmi_error()!=null&&!"".equals(host.getIpmi_error())){
                update.getParams().setIpmi_error(host.getIpmi_error());
			}
			if(host.getIpmi_errors_from()!=null&&!"".equals(host.getIpmi_errors_from())){
                update.getParams().setIpmi_errors_from(host.getIpmi_errors_from());
			}
			if(host.getIpmi_password()!=null&&!"".equals(host.getIpmi_password())){
                update.getParams().setIpmi_password(host.getIpmi_password());
			}
			if(host.getIpmi_privilege()!=null){
                update.getParams().setIpmi_privilege(host.getIpmi_privilege());
			}
			if(host.getIpmi_username()!=null&&!"".equals(host.getIpmi_username())){
                update.getParams().setIpmi_username(host.getIpmi_username());
			}
			if(host.getJmx_available()!=null){
                update.getParams().setJmx_available(host.getJmx_available());
			}
			if(host.getJmx_disable_until()!=null&&!"".equals(host.getJmx_disable_until())){
                update.getParams().setJmx_disable_until(host.getJmx_disable_until());
			}
			if(host.getJmx_error()!=null&&!"".equals(host.getJmx_error())){
                update.getParams().setJmx_error(host.getJmx_error());
			}
			if(host.getJmx_errors_from()!=null&&!"".equals(host.getJmx_errors_from())){
                update.getParams().setJmx_errors_from(host.getJmx_errors_from());
			}
			if(host.getMaintenance_from()!=null&&!"".equals(host.getMaintenance_from())){
                update.getParams().setMaintenance_from(host.getMaintenance_from());
			}
			if(host.getMaintenance_status()!=null){
                update.getParams().setMaintenance_status(host.getMaintenance_status());
			}
			if(host.getMaintenance_type()!=null){
                update.getParams().setMaintenance_type(host.getMaintenance_type());
			}
			if(host.getMaintenanceid()!=null&&!"".equals(host.getMaintenanceid())){
                update.getParams().setMaintenanceid(host.getMaintenanceid());
			}
			if(host.getName()!=null&&!"".equals(host.getName())){
                update.getParams().setName(host.getName());
			}
			if(host.getProxy_hostid()!=null&&!"".equals(host.getProxy_hostid())){
                update.getParams().setProxy_hostid(host.getProxy_hostid());
			}
			if(host.getSnmp_available()!=null){
                update.getParams().setSnmp_available(host.getSnmp_available());
			}
			if(host.getSnmp_disable_until()!=null&&!"".equals(host.getSnmp_disable_until())){
                update.getParams().setSnmp_disable_until(host.getSnmp_disable_until());
			}
			if(host.getSnmp_error()!=null&&!"".equals(host.getSnmp_error())){
                update.getParams().setSnmp_error(host.getSnmp_error());
			}
			if(host.getSnmp_errors_from()!=null&&!"".equals(host.getSnmp_errors_from())){
                update.getParams().setSnmp_errors_from(host.getSnmp_errors_from());
			}
			if(host.getStatus()!=null&&!"".equals(host.getStatus())){
                update.getParams().setStatus(host.getStatus());
			}
			if(hostInterface!=null&&hostInterface.getIp()!=null&&!"".equals(hostInterface.getIp())){
				HostInterface hif = new HostInterface();
				if(hostInterface.getType()!=null){
                ...
                update.getParams().getInterfaces().add(hif);
			}else{
				return "Error,HostInterface is required";
			}
			if(hostgroup!=null){
                ...
                update.getParams().getGroups().add(group);
			}else{
				return "Error,HostGroup is required";
			}
			if(template!=null){
                ...
                update.getParams().getTemplates().add(mytemplate);
			}else{
				return "Error,Template is required";
			}
			if(macro!=null){
                ...
                update.getParams().getMacros().add(mymacro);
			}else{
				return "Error,Macro is required";
			}
			JSONObject object = (JSONObject) update(update);
			if (object.has("result")) {
				return "success";
			}else {
				try {
